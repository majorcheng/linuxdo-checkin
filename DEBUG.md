...
- Conflicts: 还没有在 GitHub Actions 上直接验证 headed + real Chrome 是否能消除 challenge。
...
- 进行中：将 GitHub Actions 浏览器运行模式切到 `headed + real Chrome + xvfb-run`，验证是否能让点赞请求脱离 Cloudflare challenge。

## Root Cause

- 当前已确认的根因分两层：第一层是脚本构造请求链路会被 challenge，因此必须走候选楼层真实按钮点击；第二层是 GitHub Actions 里的默认 headless Chromium 环境，即使走真实按钮点击，也仍可能触发 Cloudflare challenge。第二层仍待 headed + real Chrome 的现场回归进一步确认。
...
- 进行中：为浏览器启动增加显式 `LINUXDO_BROWSER_HEADLESS` / `LINUXDO_BROWSER_REAL_CHROME` 开关，并准备在 GitHub Actions 中切到 `xvfb-run + headed + real Chrome` 进行现场验证。
